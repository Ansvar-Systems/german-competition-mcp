...
#
# The image bakes /app/data/bundeskartellamt.db at build time. Override
# BKARTA_DB_PATH for a custom location at runtime.
# ─────────────────────────────────────────────────────────────────────────────
...
# Bake the pre-built database into the image so /app/data/bundeskartellamt.db
# resolves at runtime without a bind mount.
#
...
# We then COPY it into the image at /app/data/bundeskartellamt.db
# (BKARTA_DB_PATH). The explicit `data/<name>.db` reference is required for
# the workflow's grep `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/bundeskartellamt.db
